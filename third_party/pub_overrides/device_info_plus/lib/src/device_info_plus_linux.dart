...
class DeviceInfoPlusLinuxPlugin extends DeviceInfoPlatform {
  ...
  @override
  Future<BaseDeviceInfo> deviceInfo() async => _cache ??= await _getInfo();

  Future<LinuxDeviceInfo> linuxInfo() async => (await deviceInfo()) as LinuxDeviceInfo;
  ...
  Future<Map<String, String?>?> _getOsRelease() => _tryReadKeyValues('/etc/os-release').then(
      (value) async => value ?? await _tryReadKeyValues('/usr/lib/os-release'),
    );

  Future<Map<String, String?>?> _getLsbRelease() => _tryReadKeyValues('/etc/lsb-release');

  Future<String?> _getMachineId() => _tryReadValue('/etc/machine-id');

  Future<String?> _tryReadValue(String path) => _fileSystem
        .file(path)
        .readAsString()
        .then((str) => str.trim(), onError: (_) => null);

  Future<Map<String, String?>?> _tryReadKeyValues(String path) => _fileSystem
        .file(path)
        .readAsLines()
...
extension _Unquote on String {
  ...
  String unquote() => removePrefix('"').removeSuffix('"');
}

extension _KeyValues on List<String> {
  Map<String, String?> toKeyValues() => Map.fromEntries(
      map((line) {
