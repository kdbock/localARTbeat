...
class AndroidCameraCameraX extends CameraPlatform {
  ...
  @override
  Stream<CameraInitializedEvent> onCameraInitialized(int cameraId) => _cameraEvents(cameraId).whereType<CameraInitializedEvent>();
  ...
  @override
  Stream<CameraResolutionChangedEvent> onCameraResolutionChanged(int cameraId) => _cameraEvents(cameraId).whereType<CameraResolutionChangedEvent>();

  /// The camera with ID [cameraId] has started to close.
  @override
  Stream<CameraClosingEvent> onCameraClosing(int cameraId) => _cameraEvents(cameraId).whereType<CameraClosingEvent>();

  /// The camera with ID [cameraId] experienced an error.
  @override
  Stream<CameraErrorEvent> onCameraError(int cameraId) => StreamGroup.mergeBroadcast<CameraErrorEvent>(
  ...
  @override
  Stream<VideoRecordedEvent> onVideoRecordedEvent(int cameraId) => _cameraEvents(cameraId).whereType<VideoRecordedEvent>();
  ...
  Future<Iterable<VideoStabilizationMode>> getSupportedVideoStabilizationModes(
    int cameraId,
  ) async => (await _getSupportedVideoStabilizationModeMap(cameraId)).keys;
  ...
  @override
  Stream<DeviceOrientationChangedEvent> onDeviceOrientationChanged() => deviceOrientationChangedStreamController.stream;
  ...
  /// no-op.
  @override
  Future<void> prepareForVideoRecording() => Future<void>.value();
  ...
  Iterable<(MeteringPoint, MeteringMode)> _combineMeteringPoints(
    ...
    Iterable<(MeteringPoint, MeteringMode)> toMeteringPointRecords(
      ...
      MeteringMode mode,
    ) => points.map((MeteringPoint point) => (point, mode));
