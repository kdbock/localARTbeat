...
void main() {
  ...
  void setUpOverridesForTestingUseCaseConfiguration(
    ...
    PigeonOverrides.qualitySelector_from =
        fromQualitySelector ??
        ({required VideoQuality quality, FallbackStrategy? fallbackStrategy}) => MockQualitySelector();
    GenericsPigeonOverrides.observerNew =
        <T>({required void Function(Observer<T>, T) onChanged}) => Observer<T>.detached(onChanged: onChanged);
    ...
    PigeonOverrides.deviceOrientationManager_new =
        ({
          ...
          when(manager.getUiOrientation()).thenAnswer((_) async => 'PORTRAIT_UP');
          return manager;
        };
    ...
    PigeonOverrides.cameraSize_new =
        ({required int width, required int height}) => CameraSize.pigeon_detached(width: width, height: height);
    ...
    PigeonOverrides.analyzer_new =
        newAnalyzer ??
        ({required void Function(Analyzer, ImageProxy) analyze}) => MockAnalyzer();
    PigeonOverrides.imageProxyUtils_getNv21Buffer =
        getNv21BufferImageProxyUtils ??
        (int imageWidth, int imageHeight, List<PlaneProxy> planes) => Future<Uint8List>.value(Uint8List(0));
  }
  ...
  test(
    ...
    () async {
      ...
      PigeonOverrides.processCameraProvider_getInstance = () => Future<ProcessCameraProvider>.value(mockProcessCameraProvider);
      PigeonOverrides.cameraSelector_new =
          ({LensFacing? requireLensFacing, dynamic cameraInfoForFilter}) {
            switch (requireLensFacing) {
              case LensFacing.front:
      ...
      PigeonOverrides.fallbackStrategy_lowerQualityOrHigherThan =
          ({required VideoQuality quality}) => MockFallbackStrategy();
      PigeonOverrides.cameraIntegerRange_new =
          ({required int lower, required int upper}) => CameraIntegerRange.pigeon_detached(lower: 0, upper: 0);
      ...
      () async {
        ...
        PigeonOverrides.videoRecordEventListener_new =
            ({
              ...
              onEvent,
            }) => VideoRecordEventListener.pigeon_detached(onEvent: onEvent);
    ...
    test(
      ...
      () async {
        ...
        final videoCaptureOptions = VideoCaptureOptions(
          cameraId,
          streamCallback: imageDataCompleter.complete,
        );
  ...
  test(
    ...
    () async {
      ...
      // Tell plugin to create detached Analyzer for testing.
      PigeonOverrides.analyzer_new =
          ({required void Function(Analyzer, ImageProxy) analyze}) => Analyzer.pigeon_detached(analyze: analyze);
  ...
  test(
    ...
    () async {
      ...
      onStreamedFrameAvailableSubscription = camera
          .onStreamedFrameAvailable(cameraId)
          .listen(imageDataCompleter.complete);
  ...
  test(
    ...
    () async {
      ...
      setUpOverridesForTestingUseCaseConfiguration(
        mockProcessCameraProvider,
        newAnalyzer: ({required void Function(Analyzer, ImageProxy) analyze}) => Analyzer.pigeon_detached(analyze: analyze),
        newImageAnalysis:
            ({
              int? outputImageFormat,
              ResolutionSelector? resolutionSelector,
    ...
    () async {
      ...
      PigeonOverrides.deviceOrientationManager_new =
          ({
            ...
            final manager = MockDeviceOrientationManager();
            when(manager.getDefaultDisplayRotation()).thenAnswer((_) async => defaultTargetRotation);
            return manager;
          };
